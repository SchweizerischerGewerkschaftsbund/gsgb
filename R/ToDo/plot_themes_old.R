...
#' @export
#' @rdname ggtheme
theme_sgb <- function(base_size = 12, base_family = "",
  ...
  ggplot2::theme_grey(
    ...
    ggplot2::theme(
      # axis text
      axis.text =          ggplot2::element_text(size = ggplot2::rel(1.2), colour = "black"), # axis text color and size
      axis.ticks =         ggplot2::element_line(colour = "black"), # axis ticks color
      axis.title.x =       ggplot2::element_text(margin = margin(t = half_line), vjust = 1, size = ggplot2::rel(1.2)), # Angepasst
      axis.title.y =       ggplot2::element_text(angle = 90, margin = ggplot2::margin(r = half_line),
                                                 vjust = 1, size = ggplot2::rel(1.2)), # larger y-axis title
      # axes lines (black)
      ...
      legend.text =        ggplot2::element_text(size = ggplot2::rel(1.2)), # larger legend text
      legend.position =    "top", # legend at the bottom of graph
      # -> this often has to be changed manually
      legend.direction =   "horizontal",
      legend.justification = c("center", "top"),
      # legend.box.background = ggplot2::element_rect(colour = "black", size = ggplot2::rel(1)), # border around legend
      legend.box =         "vertical",
      ...
      strip.text =         ggplot2::element_text(colour = "black", size = ggplot2::rel(1.2),
      ...
      # panel.grid.major =   ggplot2::element_line(colour = "grey50", size = ggplot2::rel(1.2), linetype = "dotted"), # from white to grey50, size = 1, linetype = "dotted"
      panel.grid.major =   ggplot2::element_line(colour = "grey80"), # grey gridlines
      panel.grid.minor =   ggplot2::element_blank(), # only major grid
      plot.margin =        ggplot2::margin(base_size, base_size, base_size, base_size), # margins around plot

      # Plot Title (ev. noch aendern)
...
#' @export
#' @rdname ggtheme
theme_sgb_blank <- function(base_size = 12, base_family = "",
                            base_line_size = base_size / 22,
                            base_rect_size = base_size / 22) {
